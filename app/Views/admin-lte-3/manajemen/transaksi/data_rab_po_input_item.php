...
<?php $psnToast = session()->getFlashdata('transaksi_toast'); ?>
<?php helper('general'); ?>

<!-- Item box -->
<?php if (!empty($SQLItem)) { ?>
    <?php echo form_open(base_url('transaksi/rab/cart_simpan_po.php'), 'autocomplete="off"') ?>
    <?php echo form_hidden('id_rab', $SQLRab->id); ?>
    <?php echo form_hidden('id_rab_det', $request->getVar('id_item_det')); ?>
    ...
    <div class="card">
        ...
        <div class="card-footer">
            <div class="row">
                <div class="col-lg-6">
                    <button type="button" class="btn btn-primary btn-flat" onclick="window.location.href = '<?php echo base_url('transaksi/rab/data_rab_aksi.php?act=po_input&id=' . $request->getVar('id') . '&status=' . $request->getVar('status') . '&id_po=' . $request->getVar('id_po')) ?>'"><i class="fas fa-arrow-left"></i> Kembali</button>
                </div>
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">DATA ITEM PO</h3>
    </div>
    <div class="card-body">                                
        <table class="table table-striped">
            ...
            <tbody>
                <?php if (!empty($SQLPORwDet)) { ?>
                    <?php
                    $no = 1;
                    $subt = 0;
                    ?>
                    <?php foreach ($SQLPORwDet as $det) { ?>
                        <tr>
...
<!-- /.card -->
<?php if (!empty($psnToast)) { ?>
    <script type="text/javascript">$(function () { <?php echo $psnToast; ?> });</script>
<?php } ?>
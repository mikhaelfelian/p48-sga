...
<?php // if ($SQLRab->status < '6') { ?>
    <!-- Item box -->
    <?php echo form_open(base_url('transaksi/set_trans_po.php'), 'autocomplete="off"') ?>
                ...
                <?php foreach ($SQLPO as $det) { ?>
                    <tr>
                        ...
                        <td>
                            <?php if (empty($SQLRab->id)) { ?>
                                <?php if ($det->status_fkt == 0) { ?>
                                    <?php echo anchor(base_url('transaksi/data_penjualan_aksi.php?act=po_input&id=' . $request->getVar('id') . '&status=' . $request->getVar('status') . '&id_po=' . $det->id), 'Input &raquo;', 'class="btn btn-primary btn-flat btn-xs" style="width: 60px;"') ?>
                                <?php } ?>
                            <?php } ?>
